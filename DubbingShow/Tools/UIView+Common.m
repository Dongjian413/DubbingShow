...
+ (UIView *)lineViewWithPointYY:(CGFloat)pointY andColor:(UIColor *)color andLeftSpace:(CGFloat)leftSpace{
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(leftSpace, pointY, kScreenWidth - kPaddingLeftWidth, 0.5)];
...
- (void)addLineUp:(BOOL)hasUp andDown:(BOOL)hasDown andColor:(UIColor *)color{
    ...
    if (hasUp) {
        UIView *upView = [UIView lineViewWithPointYY:-0.5 andColor:color];
...
- (void)addLineUp:(BOOL)hasUp andDown:(BOOL)hasDown andColor:(UIColor *)color andLeftSpace:(CGFloat)leftSpace{
    ...
    if (hasDown) {
        UIView *downView = [UIView lineViewWithPointYY:CGRectGetMaxY(self.bounds) andColor:color andLeftSpace:leftSpace];
        downView.tag = kTagLineView;
        [self addSubview:downView];
    }
}
...
- (void)widthToFitWithAdditionalWidth:(CGFloat)addWidth {
    ...
    [self sizeToFit];
    [self setSize:CGSizeMake(self.width+addWidth, originSize.height)];
}
